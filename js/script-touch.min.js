 ...
 */
function(){!function($){"use strict";$(function(){var swipewidth,width;width=$(window).width(),swipewidth=Math.floor(.16*width),swipewidth>150&&(swipewidth=150),50>swipewidth&&(swipewidth=50),$(".portfolio .project").swipe({swipeLeft:function(){},swipeRight:function(){},triggerOnTouchEnd:!1,threshold:swipewidth})})}(jQuery)}.call(this),function(){var $editor,addAudio,addElement,addH1,addH2,addImage,addLink,addOlist,addPlaceholder,addQuote,addURL,addUlist,addVideo,addWrapper,assetBase,changed,checkPlaceholderPosition,clearCursorMarker,clearSelectionMarkers,currentPlaceholderPosition,getFirstRange,getSelectedText,handleDrop,handleHover,handlePaste,handleSelection,hasElement,hideAddBar,hideAddButton,hideTextEditBox,makeSelectionWrappable,movePlaceholder,moveTimer,normalizeText,removeLink,removeWrapper,sanitize,sanitizeIfChanged,savedSelection,selectNewObject,setCursorMarker,setSelectionMarker,showAddButton,showTextEditBox,surroundRange,throttle,toggleAddBar,toggleTag;assetBase=window.assets?window.assets:"",savedSelection=void 0,$editor=$(".editContainer"),addPlaceholder=$(".addPlaceholder"),currentPlaceholderPosition=null,moveTimer=null,$(document).ready(function(){rangy.init(),$("#addLink").modal({show:!1}),setInterval(sanitizeIfChanged,100),$(document).on("mouseup",handleSelection).on("keyup",handleSelection).on("keydown",function(){var changed;changed=!0}).on("paste",handlePaste).on("drop",handleDrop),$(".editable").on("mouseover","> *",function(e){return handleHover(e)}).on("mousemove","> *",throttle(function(e){handleHover(e)},100)),$(".add-bold").on("click",function(){return toggleTag("strong")}),$(".add-italic").on("click",function(){return toggleTag("em")}),$(".add-link").on("click",function(){return addLink("temporaryLink")}),$("#addLink .save").on("click",function(){return addURL("temporaryLink",$("#linkInput").val())}),$("#addLink .cancel").on("click",function(){return removeLink("temporaryLink")}),$("#addLink #linkInput").on("keyup",function(e){return 13===e.keyCode?addURL("temporaryLink",$("#linkInput").val()):void 0}),$(".addPlaceholder .add-item").on("click",toggleAddBar),$(".addPlaceholder .add-image").on("click",addImage),$(".addPlaceholder .add-video").on("click",addVideo),$(".addPlaceholder .add-audio").on("click",addAudio),$(".addPlaceholder .add-h1").on("click",addH1),$(".addPlaceholder .add-h2").on("click",addH2),$(".addPlaceholder .add-quote").on("click",addQuote),$(".addPlaceholder .add-olist").on("click",addOlist),$(".addPlaceholder .add-ulist").on("click",addUlist)}),handleSelection=function(){getSelectedText()&&showTextEditBox()},handleHover=function(e){var addBar,bot,height,margin,my,nearBot,nearTop,parent,pos,t,top;t=$(e.currentTarget),parent=t.hasClass("editable"),addBar=t.hasClass("addPlaceholder"),parent||addBar?showAddButton():(height=t.height(),pos=t.offset(),top=pos.top,bot=pos.top+height,margin=parseInt(t.css("margin-bottom")),my=e.pageY,nearTop=my>top&&top+height/2>my&&top+100>my,nearBot=bot+margin>my&&my>bot-height/2&&my>bot-125,nearTop?checkPlaceholderPosition(t,"top"):nearBot?checkPlaceholderPosition(t,"bottom"):hideAddButton())},handlePaste=function(){return setTimeout(sanitize,0),!0},handleDrop=function(){return setTimeout(sanitize,0),!0},sanitize=function(){$editor.find("meta").detach(),$editor.find("span,font").contents().unwrap(),$editor.find("b").contents().unwrap().wrap("<strong />"),$editor.find("i").contents().unwrap().wrap("<em />"),$editor.find(".addPlaceholder").each(function(){return"true"!==$(this).attr("contenteditable")?$(this).detach():void 0}),$editor.find("strong + strong, em + em").each(function(){var $prev,prevContents;return $prev=$(this).prev(),prevContents=$prev.html(),$prev.detach(),$(this).html(prevContents+$(this).html())}),$editor.find("[style]").removeAttr("style")},changed=!1,sanitizeIfChanged=function(){changed&&(sanitize($editor),changed=!1)},checkPlaceholderPosition=function(el,location){var moveBottom,moveTop;moveTop="top"===location&&el.index()-1!==currentPlaceholderPosition,moveBottom="bottom"===location&&el.index()+1!==currentPlaceholderPosition,(moveTop||moveBottom)&&null===moveTimer?addPlaceholder.hasClass("showButton")?(hideAddButton(),moveTimer=setTimeout(function(){movePlaceholder(el,location),moveTimer=null},200)):movePlaceholder(el,location):null===moveTimer&&showAddButton()},movePlaceholder=function(el,location){"top"===location?addPlaceholder.detach().insertBefore(el):"bottom"===location&&addPlaceholder.detach().insertAfter(el),currentPlaceholderPosition=addPlaceholder.index()},showAddButton=function(){addPlaceholder.hasClass("showButton")||addPlaceholder.addClass("showButton")},hideAddButton=function(){addPlaceholder.removeClass("showButton"),addPlaceholder.removeClass("showBar")},toggleAddBar=function(){return addPlaceholder.hasClass("showBar")?hideAddBar():addPlaceholder.addClass("showBar")},hideAddBar=function(){return addPlaceholder.removeClass("showBar")},addImage=function(){return addElement("<img />")},addVideo=function(){return addElement('<div class="video"></div>')},addAudio=function(){return addElement('<div class="audio"></div>')},addH1=function(){return addElement("<h1>[title]</h1>")},addH2=function(){return addElement("<h2>[title]</h2>")},addQuote=function(){return addElement("<blockquote>[quote]</blockquote>")},addOlist=function(){return addElement("<ol><li></li></ol>")},addUlist=function(){return addElement("<ul><li></li></ul>")},addElement=function(newEl){var el,newClass;hideAddBar(),el=$(newEl),newClass="newEl",el.addClass(newClass),$(".addPlaceholder").after(el),selectNewObject(newClass)},selectNewObject=function(className){var newRange,node;newRange=rangy.createRange(),node=$("."+className).get(0),newRange.selectNodeContents(node),rangy.getSelection().setSingleRange(newRange),$("."+className).removeClass(className)},toggleTag=function(tag,newClass){var addTag,curr,range,tagSelector,thisClass;return range=addWrapper(),thisClass=newClass?"."+newClass:"",tagSelector=tag+thisClass,curr=$(".currentSelection"),addTag=!hasElement(curr,tagSelector,!0),addTag&&surroundRange(tag,newClass,range),removeWrapper(),rangy.restoreSelection(savedSelection),addTag},hasElement=function(el,testEl,remove){var contains,hasTestElement,wrapped;return wrapped=el.closest(testEl).size()>0,contains=el.find(testEl).size()>0,hasTestElement=wrapped||contains,hasTestElement&&remove&&(wrapped&&el.closest(testEl).contents().unwrap(),contains&&el.find(testEl).contents().unwrap()),hasTestElement},addURL=function(identifier,href){var classString;classString=identifier?"."+identifier:"",$("#addLink").modal("hide"),$("a"+classString).attr("href",href).removeClass(identifier),$("#linkInput").val(""),removeWrapper()},removeLink=function(){$("#addLink").modal("hide"),$("a.temporaryLink").contents().unwrap(),rangy.restoreSelection(savedSelection),removeWrapper()},addLink=function(newClass){var add;return addWrapper(),add=!hasElement($(".currentSelection"),"a",!0),add&&(toggleTag("a",newClass),$("#addLink").modal("show"),window.setTimeout(function(){return $("#addLink #linkInput").focus(),console.log("Focus, please")},250)),removeWrapper(),add},addWrapper=function(){var range;return range=getFirstRange(),surroundRange("span","currentSelection",range),range},removeWrapper=function(){$(".currentSelection").contents().unwrap()},hideTextEditBox=function(){$(".textEditBar").addClass("hidden")},showTextEditBox=function(){var editBar,left,sPos,selectionMarker,top;selectionMarker=$(".rangySelectionBoundary").css("display","inline").first(),editBar=$(".textEditBar"),sPos=0,top=0,left=0,selectionMarker?(editBar.removeClass("hidden"),sPos=selectionMarker.position(),top=sPos.top-editBar.height()-15,left=sPos.left-15,$(".textEditBar").css("top",top).css("left",left)):$(".textEditBar").hide()},getSelectedText=function(){var hasSelection,sel,selectedText;return sel=rangy.getSelection(),selectedText=sel.toString(),hasSelection=!1,selectedText?hasSelection=setSelectionMarker():(hideTextEditBox(),hasSelection=setCursorMarker()),hasSelection},setCursorMarker=function(){var cursorActive,parent,range;return range=getFirstRange(),cursorActive=!0,range&&(parent=$(range.startContainer),cursorActive=parent.closest(".editable").size()>0),clearSelectionMarkers(),!1},clearCursorMarker=function(){$(".cursorSelection").remove()},clearSelectionMarkers=function(){clearCursorMarker(),savedSelection&&(rangy.removeMarkers(savedSelection),savedSelection=null,normalizeText())},setSelectionMarker=function(){return clearSelectionMarkers(),savedSelection=rangy.saveSelection(),!0},getFirstRange=function(){var sel;return sel=rangy.getSelection(),sel.rangeCount?sel.getRangeAt(0):void 0},normalizeText=function(){var eCon,range,sCon;range=getFirstRange(),sCon=range.startContainer,eCon=range.endContainer;try{sCon.parentNode.normalize(),eCon.parentNode.normalize()}catch(_error){}},surroundRange=function(newElement,elClass,inputRange){var el,elementClass,ex,range;if(range=inputRange?inputRange:getFirstRange(),elementClass=elClass?elClass:"",range){el=document.createElement(newElement),el.className=elementClass;try{range.surroundContents(el)}catch(_error){ex=_error,console.log((ex instanceof rangy.RangeException||"[object RangeException]"===Object.prototype.toString.call(ex))&&1===ex.code?"Unable to surround range because range partially selects a non-text node. See DOM Level 2 Range spec for more information.\n\n"+ex:"Unexpected errror: "+ex)}}rangy.getSelection().setSingleRange(range),range.refresh()},makeSelectionWrappable=function(inputRange){var range;return range=inputRange?inputRange:getFirstRange(),range.canSurroundContents()?range:(console.log("Can't surround contents"),console.log(range.startOffset),console.log(range.endOffset),range.setStartBefore(range.startContainer),range.setEndAfter(range.endContainer),console.log(range.startOffset),console.log(range.endOffset),console.log(range),range)},throttle=function(fn,threshold,scope){var deferTimer,last;return threshold=threshold?threshold:250,last=void 0,deferTimer=void 0,function(){var args,context,now;context=scope||this,now=(new Date).getTime(),args=arguments,last&&last+threshold>now?(clearTimeout(deferTimer),deferTimer=setTimeout(function(){last=now,fn.apply(context,args)},threshold)):(last=now,fn.apply(context,args))}}}.call(this);